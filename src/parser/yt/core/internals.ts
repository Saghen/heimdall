...
export const headOfSome = pipe(someToArray, head) as <T, U, V>(value: Some<SomeOptions<T, U>>) => T | U

export type Runs<T> = { runs: T[] }
export type Some<T extends SomeOptions<{}, {}> | SubCommand> = T extends SomeOptions<infer Single, infer Many>
  ? Single | Runs<Many>
  : never
...
// Renderer
// export const unwrapRenderer = <Name extends string, T extends Record<string, any>>(
//   renderer: Renderer<Name, T>
// ): T => 'asd'
...
export const isRenderer =
  <Name extends string, MaybeRenderer extends Renderer<Name>>(name: Name) =>
  (renderer: Renderer): renderer is MaybeRenderer =>
